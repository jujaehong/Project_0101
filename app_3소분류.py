...
def run_app_3소분류():
    ...
    if  st.header('불량유형 소분류별 A/S 접수 데이터') :
        ...
        if not all_소분류_selected:
            selection_label = "**특정 소분류 유형 데이터만 보고 싶을 경우 아래에서 선택하세요**"
            selected_소분류_list = st.multiselect(selection_label + ":", 소분류_list, default=[])
            if len(selected_소분류_list) == 0:
